...
async function runTests (runType, t) {
  ...
  t.test(`${mode} new scheduled`, async t => {
    ...
    i = await getInv(t, cwd)
    t.equal(i.inv.lambdaSrcDirs.length, 1 + defaultNoOfLambdas, 'Project now has two Lambdas')
    ...
    i = await getInv(t, cwd)
    t.equal(i.inv.lambdaSrcDirs.length, 2 + defaultNoOfLambdas, 'Project now has three Lambdas')
