...
async function runTests (runType, t) {
  ...
  t.test(`${mode} scaffold project - normal`, async t => {
    ...
    t.equal(i.inv.lambdaSrcDirs.length, defaultNoOfLambdas, 'Project has default number of Lambda')
  ...
  t.test(`${mode} scaffold project - reference file based`, async t => {
    ...
    i = await getInv(t, cwd)
    t.equal(i.inv.lambdaSrcDirs.length, defaultNoOfLambdas, 'Project still has one Lambda')
