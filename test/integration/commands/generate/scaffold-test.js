...
async function runTests (runType, t) {
  ...
  t.test(`${mode} scaffold project - normal`, async t => {
    ...
    t.ok(existsSync(join(cwd, 'models/books.mjs')), 'Wrote books db code')
    ...
    t.ok(existsSync(join(cwd, 'models/schemas/book.mjs')), 'Wrote books JSON schema')
  ...
  t.test(`${mode} scaffold project - file based`, async t => {
    ...
    t.ok(existsSync(join(cwd, 'models/schemas/person.mjs')), 'Wrote person JSON schema')
    t.notOk(r.stderr, 'Did not print to stderr')
    t.equal(r.code, 0, 'Exited 0')
  })
  ...
  t.test(`${mode} scaffold project - reference file based`, async t => {
    ...
    t.ok(existsSync(join(cwd, 'models/customers.mjs')), 'Wrote customers db code')
    ...
    t.ok(existsSync(join(cwd, 'models/schemas/customer.mjs')), 'Wrote customer JSON schema')
    t.notOk(r.stderr, 'Did not print to stderr')
    t.equal(r.code, 0, 'Exited 0')
  })
